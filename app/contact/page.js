...
const ContactSchema = Yup.object().shape({
  ...
  message: Yup.string().min(10, "Please write more than that.").required("Required"),
})
			  ...
			  <div>
				...
				<Field
				  ...
				  className={`${form.input} ${
					errors.fullName && touched.fullName ? form.error : ""
				  }`}
				/>
				{errors.fullName && touched.fullName && (
				  <div className={form.errorMessage}>{errors.fullName}</div>
				)}
			  </div>
			  <div>
				...
				{errors.email && touched.email && (
				  <div className={form.errorMessage}>{errors.email}</div>
				)}
			  </div>
			  <div>
				...
				{errors.message && touched.message && (
				  <div className={form.errorMessage}>{errors.message}</div>
				)}
			  </div>
